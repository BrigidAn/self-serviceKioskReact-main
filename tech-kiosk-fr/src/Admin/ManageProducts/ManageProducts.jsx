...
export default function ManageProducts() {
  ...
  const validateForm = () => {
    ...
    if (!form.quantity && form.quantity !== 0) newErrors.quantity = "Quantity is required";
    else if (form.quantity < 0) newErrors.quantity = "Quantity cannot be negative";
    if (!form.category || form.category === "") newErrors.category = "Please select a category";
    if (!form.supplierId) newErrors.supplierId = "Please select a supplier";
    if (!isEditing && !fileRef.current?.files?.length) newErrors.image = "Product image is required";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing && editingId) {
        ...
        // Adding new product
        if (!form.file) return toast.error("Please select a product image");
        const fd = new FormData();
        fd.append("Name", form.name.trim());
        fd.append("Description", form.description || "");
        fd.append("Price", Number(form.price));
          ...
          <div className="modal-overlay">
            <div className="modal">
              ...
              <form className="modal-form" onSubmit={handleSubmit}>
                ...
                <label>
                  Category
                  <select className="c-form-select" name="category" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>
                ...
                <label>
                  Supplier
                  <select className="c-form-select" name="supplierId" value={form.supplierId} onChange={(e) => setForm({ ...form, supplierId: Number(e.target.value) })}>
